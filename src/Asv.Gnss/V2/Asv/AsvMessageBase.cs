...
namespace Asv.Gnss.V2
{
    public abstract class AsvMessageBase:GnssMessageBase<ushort>
    {
        ...
        public override void Deserialize(ref ReadOnlySpan<byte> buffer)
        {
            ...
            {
                throw new Exception($"Error to deserialize {nameof(AsvMessageBase)}");
            }
            var length = BinSerialize.ReadUShort(ref buffer);
            var crc = SbfCrc16.Calc(crcSpan, length + 10);
            crcSpan = crcSpan.Slice(length + 10);
            var crcOrigin = BinSerialize.ReadUShort(ref crcSpan);
            if (crc != crcOrigin)
            {
                throw new Exception($"Error to deserialize {nameof(AsvMessageBase)}: CRC error. Want {crc}. Got {crcOrigin}");
            }
            ...
            if (MessageId != msgId)
            {
                throw new Exception($"Error to deserialize {nameof(AsvMessageBase)}: Message id not equals. Want '{MessageId}. Got '{msgId}''");
            }
        ...
        public override void Serialize(ref Span<byte> buffer)
        {
            ...
            var crc = SbfCrc16.Calc(originSpan, length + 10 /*from sync1 to end of data*/);
